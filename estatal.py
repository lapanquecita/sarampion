...
def crear_mapa(año, entidad):
    ...
    df = df[df["CVE"].str.startswith(str(entidad).zfill(2))]
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                ...
                xanchor="center",
                yanchor="top",
                text=f"Tasas de incidencia de sarampión en <b>{ENTIDADES[entidad]}</b> durante {año}",
                font_size=40,
            ),
            dict(
                x=0.06,
...
def crear_tabla_absolutos(año, entidad):
    ...
    fig.update_layout(
        ...
        title_y=0.95,
        title_font_size=40,
        title_text=f"Los 30 municipios de <b>{ENTIDADES[entidad]}</b> con la mayor<br><b>incidencia</b> de sarampión durante {año}",
        paper_bgcolor=PAPER_COLOR,
        annotations=[
            dict(
                x=0.015,
